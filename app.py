...
@app.route('/api/webhook/share', methods=['POST'])
def webhook_share():
    """
    Scraper'dan gelen ilanları otomatik paylaş
    
    Headers:
        X-Webhook-Secret: Secret key (WEBHOOK_SECRET env var ile eşleşmeli)
    
    Body (JSON):
        Single: {"baslik": "...", "gorsel": "...", "aciklama": "...", "konum": "..."}
        Array: [{"baslik": "..."}, {"baslik": "..."}]
    """
    try:
        # Secret key kontrolü
        ...
        # Array ise tek tek işle
        ilanlar = data if isinstance(data, list) else [data]
        results = []
        
        for ilan in ilanlar:
            # Zorunlu alanlar
            baslik = ilan.get('baslik')
            gorsel_url = ilan.get('gorsel')
            
            if not baslik or not gorsel_url:
                results.append({
                    "success": False,
                    "ilan": baslik or "N/A",
                    "error": "baslik ve gorsel zorunludur"
                })
                continue
            ...
            # Hashtag ekle (webhook için varsayılan açık)
            ...
            try:
                ...
                # Aktif platformlarda paylaş
                ilan_results = {}
                
                if os.getenv('FACEBOOK_ACCESS_TOKEN'):
                    fb_result = fb_publisher.publish(message_with_tags, temp_file.name)
                    ilan_results['facebook'] = fb_result
                
                if os.getenv('TWITTER_API_KEY'):
                    tw_result = tw_publisher.publish(message_with_tags, temp_file.name)
                    ilan_results['twitter'] = tw_result
                
                if os.getenv('INSTAGRAM_ACCESS_TOKEN'):
                    ig_result = ig_publisher.publish(message_with_tags, gorsel_url)
                    ilan_results['instagram'] = ig_result
                ...
                results.append({
                    "success": any(r.get('status') == 'success' for r in ilan_results.values()),
                    "ilan": baslik[:50] + "...",
                    "results": ilan_results
                })
